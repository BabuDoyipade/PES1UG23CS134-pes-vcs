...
// Recursive helper
static int write_tree_recursive(IndexEntry *entries, int count, const char *prefix, ObjectID *out_id) {
    Tree tree;
    tree.count = 0;
    ...
    while (i < count) {
        ...
        const char *subpath = prefix ? path + strlen(prefix) : path;
        ...
        if (!slash) {
            ...
            // Collect entries inside this directory
            ...
            while (j < count) {
                ...
                if (prefix && strncmp(p, prefix, strlen(prefix)) != 0)
                    break;

                const char *sub = prefix ? p + strlen(prefix) : p;

                if (strncmp(sub, dirname, len) == 0 && sub[len] == '/') {
            ...
            if (write_tree_recursive(sub_entries, sub_count, new_prefix, &sub_tree_id) != 0)
                return -1;
